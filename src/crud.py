...
def get_preference(db: Session, user_id: int):
    return (
        ...
        .filter(models.Preferences.user_id == user_id)
        .first()
    )


def create_preference(db: Session, preference: schemas.PreferenceCreate):
    db_preference = models.Preferences(
...
def update_preference(
    db: Session,
    user_preferences: schemas.Preference,
    new_preferences: schemas.PreferenceUpdate,
):
    stored_item = schemas.Preference(
    ...
    db.query(models.Preferences).filter(
        models.Preferences.user_id == user_preferences.user_id
